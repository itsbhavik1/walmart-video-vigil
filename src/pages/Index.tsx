...
const Index = () => {
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden flex flex-col bg-transparent">
      <AnimatedBackground />
      <WalmartHeader />
      <main className="flex flex-col flex-1 py-12 md:px-24 px-2 items-center gap-y-8 w-full">
        <h2 className="text-3xl md:text-5xl font-extrabold text-[#0071ce] mt-6 mb-2 text-center tracking-tight drop-shadow-lg font-sans animate-fade-in">
          Fraudulent Activity Video Detection
        </h2>
        <div className="flex justify-center items-center gap-16 w-full animate-fade-in">
          <p className="text-base md:text-xl text-gray-700 font-light bg-white/60 rounded-xl px-7 py-4 shadow-lg backdrop-blur-sm max-w-2xl border-l-4 border-[#ffc220]">
            Upload your in-store footage and let our advanced AI detect security threats in seconds.<br className="hidden md:inline" />
            <span className="text-[#ffc220] font-semibold underline underline-offset-2 decoration-dotted decoration-[#ffc220]/70">
              Trusted for modern retail vigilance.
            </span>
          </p>
        </div>
        <section className="w-full max-w-2xl mx-auto rounded-3xl bg-white/85 backdrop-blur-2xl shadow-2xl p-8 border-2 border-[#0071ce26] animate-fade-in scale-100 hover:scale-[1.012] transition duration-200 card-glass">
          ...
          <div className="flex flex-row flex-wrap justify-between items-center gap-2 mt-4">
            <Button
              className="bg-[#ffc220] hover:bg-[#ffd966] text-[#0071ce] font-semibold shadow-lg shadow-[#ffc2203c] transition-all duration-200 px-10 py-3 rounded-2xl text-lg scale-100 hover:scale-105 focus:scale-95"
              size="lg"
            ...
            {previewUrl && (
              <span className="ml-2 text-xs bg-blue-100 px-3 py-1 rounded shadow text-[#0071ce] animate-fade-in">
                Video Ready!
              </span>
            )}
          </div>
        </section>
        <section className="w-full max-w-2xl mt-2 md:mt-6 drop-shadow-xl animate-fade-in">
